...
def img_preprocess_task(step_info:dict,file_info:dict,result_key:str="result")->dict:
    ...
    file = type_convert_util.convert_type(output,before_output_type,function_map["save"]["input_type"])
    save(file,f"_result")
...
def thinner(img_np_bgr: np.ndarray) -> np.ndarray:
    ...
    return edges
def del_blank_set1(img_np_bgr: np.ndarray, padding: int = 1) -> np.ndarray:
    """
