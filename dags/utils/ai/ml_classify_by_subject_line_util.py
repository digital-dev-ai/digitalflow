...
def classify(ai_info: dict, file_info: dict, target_key: str, **context):
    ...
    pred, confidence, input_kwargs = predict(image_path, model)
    if save_input:
...
#     # csv데이터셋 만드는 곳 여기에 들어가야함. True, false 상관없음. 
...
def predict(image_path, model,output_csv_path='/opt/airflow/data/class/a_class/classify/csv'):
    """예측 수행 (머신러닝 모델용으로 수정)"""
    ...
    if output_csv_path:
        ...
        if os.path.exists(output_csv_path):
            input_df.to_csv(f'{output_csv_path}/doc_data.csv', mode='a', header=False, index=False, encoding='utf-8-sig')
