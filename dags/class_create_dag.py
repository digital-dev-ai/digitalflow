...
from tasks.class_create_task import balance_false_images, build_balanced_dataset, train, image_data_augment
...
CLASS_DIR = f"{DATA_DIR}/class"   # 클래스 관련 데이터 디렉토리
...
with DAG(
    ...
    # 1-1. AI 생성 작업 정보 로드
    d_layout_list = dococr_query_util.select_list_map("selectLayoutList",None)
    target_method = "in" # in|not in|all
    target_doc_class_ids = ["5"]
    target_layout_class_ids = ["9"]
    
    for d_layout_info in d_layout_list:
        ...
        if target_method == "in": # 목록 안의 대상만 작업
            if d_doc_class_id not in target_doc_class_ids and d_layout_class_id not in target_layout_class_ids: 
                continue
        elif target_method == "not in": # 목록에 없는 대상만 작업
            if d_doc_class_id in target_doc_class_ids and d_layout_class_id in target_layout_class_ids:
                continue
        elif target_method == "all": # 모든 대상 작업
            pass
        d_class_name = d_layout_info.get("layout_name")
        d_classify_preprocess_step_info = json.loads(d_layout_info["img_preprocess_info"])
        d_classify_ai_info = json.loads(d_layout_info["classify_ai_info"])
        d_layout_dir = f"{DATA_DIR}/class/{d_doc_class_id}/{d_layout_class_id}"
        json_util.save_json_file(f"{CLASS_DIR}/layout_list.json", d_layout_info)
        ...
        if augskip:
            ...
            augment_threshold = 200
        # 1-1. 증강 처리를 통해 파일 개수 증가
        t_true_image_data_augment = image_data_augment(d_origin_true_imgae_dir,d_ready_true_image_dir, threshold=augment_threshold)
        t_false_image_data_augment = image_data_augment(d_origin_false_imgae_dir,d_ready_false_image_dir, threshold=augment_threshold)
        ...
        # 4. 모델 학습
        t_train = train(t_build_dataset,d_classify_ai_info,ai_dir=d_ai_dir,model_name=d_model_name)
