...
with DAG(
    dag_id="block_ocr_v1", # 이전 DAG ID와 충돌 방지를 위해 변경
    ...
    #A클래스에서 분리된 영역
    STANDARD_FOLDER = "/opt/airflow/data/sample/block"
    ...
    t_ocr_task = only_ocr.partial(target_key="_origin").expand(file_info=t_target_file_info_list)
