...
OUTPUT_MODEL_DIR = f"{DATA_DIR}/class/b_class/classify/model"   #ai 모델
...
with DAG(
    dag_id='lilt_document_classifier_balanced_b_class_V0.1',
    ...
    #스텝정보 가져오기
    b_class_classify_preprocess_info = file_util.get_step_info_list("b_class","classify","img_preprocess")

    true_file_info_list_task = get_file_info_list_task(READY_TRUE_IMAGE_DIR)
    true_file_preprocess_partial_task = img_preprocess_task.partial(step_info=b_class_classify_preprocess_info,result_key="true")
    ...
    false_file_preprocess_partial_task = img_preprocess_task.partial(step_info=b_class_classify_preprocess_info,result_key="false")
