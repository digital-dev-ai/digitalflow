...
ALTER TABLE TB_DI_DOC_CLASS MODIFY COLUMN DOC_CLASS_ID INT NOT NULL AUTO_INCREMENT;

ALTER TABLE TB_DI_LAYOUT_CLASS MODIFY COLUMN  LAYOUT_CLASS_ID INT NOT NULL AUTO_INCREMENT;

ALTER TABLE TB_DI_SECTION_CLASS MODIFY COLUMN SECTION_CLASS_ID INT NOT NULL AUTO_INCREMENT;

ALTER TABLE TB_DI_BLOCK_CLASS MODIFY COLUMN BLOCK_CLASS_ID INT NOT NULL AUTO_INCREMENT;


ALTER TABLE TB_DI_BLOCK_CRCTN MODIFY COLUMN BLOCK_CRCTN_ID INT NOT NULL AUTO_INCREMENT;

ALTER TABLE TB_DI_COMMON_CRCTN MODIFY COLUMN COMMON_CRCTN_ID INT NOT NULL AUTO_INCREMENT;


CREATE TABLE `TB_DS_TABLE` (
    TABLE_ID   INT AUTO_INCREMENT PRIMARY KEY COMMENT '테이블ID',
    TABLE_NAME VARCHAR(100) NOT NULL COMMENT '테이블명',
    TABLE_DESC VARCHAR(200) COMMENT '테이블 설명'
) COMMENT='데이터 테이블';
...
CREATE TABLE `TB_OCR_FLR_STATUS` (
	...
	`FLR_AREA_SQM`	DECIMAL(10,2)	NULL COMMENT '층 면적 (제곱미터)'
);
...
CREATE TABLE `TB_OCR_BILD_BASIC_INFO` (
	...
	`LAND_AREA_SQM`	DECIMAL(10, 2)	NULL	COMMENT '대지 면적 (제곱미터)',
	`TOT_FLR_AREA_SQM`	DECIMAL(10, 2)	NULL	COMMENT '연면적 (제곱미터)',
	`ADMIN_DIST_NM`	VARCHAR(50)	NULL	COMMENT '지역명',
	`SECT_TYPE`	VARCHAR(50)	NULL	COMMENT '지구 유형',
	`ZONE_TYPE`	VARCHAR(50)	NULL	COMMENT '구역 유형',
	`BILD_AREA_SQM`	DECIMAL(10, 2)	NULL	COMMENT '건축 면적 (제곱미터)',
	`FAR_CALC_AREA_SQM`	DECIMAL(10, 2)	NULL	COMMENT '용적률 산정 연면적 (제곱미터)',
	...
	`BILD_COVERAGE_RATIO`	DECIMAL(5, 2)	NULL	COMMENT '건폐율 (%)',
	`FLR_AREA_RATIO`	DECIMAL(5, 2)	NULL	COMMENT '용적률 (%)',
	`BILD_HEIGHT_METER`	DECIMAL(6, 2)	NULL	COMMENT '건축물 높이 (미터)',
	`ROOF_STRUCT_TYPE`	VARCHAR(100)	NULL	COMMENT '지붕 구조 유형',
	`ANNEX_BILD_TYPE`	VARCHAR(50)	NULL	COMMENT '부속 건축물 유형',
	`LNDSCP_AREA_SQM`	DECIMAL(10, 2)	NULL	COMMENT '조경 면적 (제곱미터)',
	`PUB_OPEN_AREA_SQM`	DECIMAL(10, 2)	NULL	COMMENT '공개 공지 면적 (제곱미터)',
	`SETBACK_AREA_SQM`	DECIMAL(10, 2)	NULL	COMMENT '건축선 후퇴 면적 (제곱미터)',
	`SETBACK_DIST_METER`	DECIMAL(6, 2)	NULL	COMMENT '건축선 후퇴 거리 (미터)'
);
...
CREATE TABLE `TB_OCR_OWN_STATUS` (
	...
	`OWNR_CHG_DATE`	DATE	NULL COMMENT '소유권 변동일',
...
ALTER TABLE TB_OCR_FLR_STATUS MODIFY COLUMN FLR_SEQ_NUM INT NOT NULL AUTO_INCREMENT COMMENT '건축물 현황 시쿼번호';
